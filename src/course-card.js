...
const CourseCard = (props) => {
    ...
    return (
        <>
        <div className={`text-accent-600 font-bold flex justify-between w-full bg-gray-200 ${state.open ? "rounded-t" : "rounded"} p-2 mt-2`}> 
            {state.edit ? 
                <h1 ref={code} suppressContentEditableWarning={true} contentEditable="plaintext-only" className="empty:before:content-['Enter_code_here...'] focus:outline-none empty:before:text-accent-600/75 py-1 text-xl" >{props.data.code}</h1>
            :
                <>
                <button className="text-xl" onClick={() => toggleDropDown()} >{props.data.code}</button>
                <div>
        ...
        <div className={`${animation} grid grid-rows-0`}>
            <div className="bg-gray-300 overflow-hidden">
                ...
                <div className="text-right">
                    <button onClick={() => props.Save(name, description)} className='m-2 px-4 py-1 bg-primary-600 rounded-md text-lg font-bold text-white hover:bg-primary-400 inline'> 
                        <a > Save </a>
                    </button>
                    <button onClick={() => props.Cancel()} className='m-2 mr-6 px-4 py-1 bg-red-600 rounded-md text-lg font-bold text-white hover:bg-red-400 inline'> 
                        <a > Cancel </a>
                    </button>
                </div>
            </div>
